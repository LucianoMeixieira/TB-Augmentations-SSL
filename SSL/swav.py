...
class SwAV(nn.Module):
    ...
    def __init__(self, backbone, feature_size, augmentation, batch_size, projection_dim=128, hidden_dim=2048, temperature=0.1, epsilon=0.05, 
                 sinkhorn_iterations=3, num_prototypes=3000, use_the_queue=True, num_crops=6, resized_size=224, epoch_threshold=15): 
        super().__init__()
        ...
        self.queue_length = 15 * batch_size # to simulate 4096 batch size using a batch size of 256
        self.use_the_queue = use_the_queue
        self.resized_size = resized_size
        self.epoch_threshold = epoch_threshold
        self.training_epoch = 0 # initially 0
        ...
        self.augment_global = self._global()
    ...
    def _global(self):
        if self.augmentation == "default" or self.augmentation == "bone_default":
            return T.Compose([ # if default will take in none suppressed, if bone_default will take in bone suppressed
                    T.RandomResizedCrop(self.resized_size, scale=(0.14, 1.0)),
            ...
            return T.Compose([ # take in bone suppressed and none bone suppressed
    ...
    def _local(self):
        if self.augmentation == "default" or self.augmentation == "bone_default":
            return T.Compose([
                    T.RandomResizedCrop(int(self.resized_size*3/7), scale=(0.05, 0.14)),
                    T.RandomHorizontalFlip(),
                    T.RandomApply([T.ColorJitter(brightness=0.8, contrast=0.8)], p=0.8),
                    T.RandomApply([T.GaussianBlur(kernel_size=(int(self.resized_size*3/7))//20*2+1, sigma=(0.1, 2.0))], p=0.5)
                ])
    ...
    def forward(self, x, y=None): # make sure x is always bone suppressed if included
        if y is not None: 
            x1, x2 = self.augment_global(x), self.augment_global(y)
        else:
        ...
        with torch.no_grad():
            if self.queue is not None and self.use_the_queue and (self.training_epoch >= self.epoch_threshold): # only use the queue after 15 epochs
...
def swav_loss(c1, c2, c_c, q1, q2, temperature, num_crops):
    ...
    # Compute the loss for the main views
    p1, p2 = c1 / temperature, c2 / temperature
    subloss = -torch.mean(torch.sum(q1 * F.log_softmax(p2, dim=1), dim=1))
    subloss -= torch.mean(torch.sum(q2 * F.log_softmax(p1, dim=1), dim=1))
    loss += subloss / (num_crops - 1)
    
    # Compute the loss for the additional views
    for c in c_c:
        p = c / temperature
        subloss = -torch.mean(torch.sum(q1 * F.log_softmax(p, dim=1), dim=1))
        subloss -= torch.mean(torch.sum(q2 * F.log_softmax(p, dim=1), dim=1))
        loss += subloss / (num_crops - 1)
    
    # Normalize the loss by the number of crops used for assignment
    loss /= 2 
