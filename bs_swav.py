...
BATCH_SIZE = 256
EPOCHS_PRE = 200 # does benefit from longer training though
WARM_UP = 0 # epochs for linear warm up 
LEARNING_RATE_PRE = 0.6
MOMENTUM = 0.9
WEIGHT_DECAY = 0.000001 # 10e-6
FREEZE_PROTOTYPES_NITERS = 12 # freeze weights during the first epoch of training 
SEED = 42
...
def main(args):
    ...
    if args.mode == 'pretrain': 
        ...
        BASE_DIR = 'swav_pretrained_models'
        ...
        swav = SwAV(backbone=backbone, feature_size=feature_size, augmentation=args.augmentation, batch_size=BATCH_SIZE)
        logging.info(f'Created {args.augmentation} SwAV Model')
        
        # pretrain the SwAV model
        loss = pretrain(
            model=swav, 
        ...
        save_loss(loss, LOSS_DIR, 'SwAV Pre-Training Loss')

    else:
        
        # define directories
        BASE_DIR = "swav_fine_tuned_models"
        SAVE_DIR = os.path.join(BASE_DIR, args.augmentation)
        if not os.path.exists(SAVE_DIR):
            os.makedirs(SAVE_DIR)
        ...
        if args.mode == "train":
            pretrained_weights = torch.load(f'swav_pretrained_models/{args.augmentation}/weights/swav_epoch_200.pth', map_location='cpu')
            ...
            linear_weights = torch.load(f'swav_fine_tuned_models/{args.augmentation}/weights/swav_fine_tuned_linear.pth', map_location='cpu')
            full_weights = torch.load(f'swav_fine_tuned_models/{args.augmentation}/weights/swav_fine_tuned_full.pth', map_location='cpu')
...
def pretrain(model, augmentation, device, dataset, WEIGHTS_DIR):
    ...
    cosine_scheduler = CosineAnnealingLR(optimizer, T_max=(EPOCHS_PRE - WARM_UP), eta_min=0.0006)
    loss_values = []
    ...
    for epoch in range(EPOCHS_PRE):
        # Update the training epoch
        model.update_training_epoch(epoch)
        ...
        # train the model for 1 epoch and get the loss from swav
        ...
        if (epoch + 1) % 10 == 0: # only save the weights every 10 epochs
            ...
            save_path = os.path.join(WEIGHTS_DIR, f'swav_epoch_{epoch + 1}.pth')
...
def pretrain_epoch(model, optimizer, dataset, data1, data2, epoch, cosine_scheduler, device):
    ...
    if data2:
        ...
        for it, (norm_batch, sup_batch) in enumerate(zip(dataloader1, dataloader2)):
            iteration = epoch * len(dataloader1) + it
            ...
            loss.backward()
            if iteration < FREEZE_PROTOTYPES_NITERS: # freeze the prototypes at the beginning of training
        ...
        for it, batch in enumerate(dataloader1):
            ...
            # compute gradient and do SGD step
            loss.backward()
            if iteration < FREEZE_PROTOTYPES_NITERS: # freeze the prototypes
                model.freeze_prototypes()
            
            optimizer.step()
            
    # If decide to do linear warm up
    if epoch < WARM_UP:
...
def fine_tune(model, device, config, train_dataloader, val_dataloader, sweep, WEIGHTS_DIR):
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=config['lr'], momentum=MOMENTUM, weight_decay=config["wd"]) 
    ...
    save_path = os.path.join(WEIGHTS_DIR, f'swav_fine_tuned_{sweep}.pth')
    torch.save(model.state_dict(), save_path)
    logging.info(f'Model saved to {save_path}')
...
def save_fine_tune_loss(train_loss, val_loss, l_dir, sweep):
    ...
    save_loss_curve(train_loss, loss_curve_path, f"SwAV {sweep} sweep loss curves", val_loss)
...
def plot_confusion_matrix(true_labels, predicted_labels, classes, filename, augment, 
    ...
    ax.set_title(f'Confusion Matrix For SwAV {augment}', fontsize=fontsize+2)
...
def plot_auc_roc(true_labels, predicted_labels, augment, save_path):
    ...
    plt.title(f'Receiver Operating Characteristic (ROC) Curve for SwAV {augment}')
