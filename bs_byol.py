...
EPOCHS_PRE = 500
WARM_UP = 10
LEARNING_RATE_PRE = 0.4 * (BATCH_SIZE/256)
WEIGHT_DECAY = 1.5 * 10e-6
N = 4096 / BATCH_SIZE # when reducing the batch size by a factor N, we average gradients over N consecutive steps and update the target network once every N steps
SEED = 42
...
def main(args):
    ...
    if args.mode == 'pretrain': 
        ...
        BASE_DIR = 'byol_pretrained_models'
        ...
        byol = BYOL(backbone=backbone, feature_size=feature_size, augmentation=args.augmentation, device=device)
        logging.info(f'Created {args.augmentation} BYOL Model')
        
        # pretrain the BYOL model
        loss = pretrain(
            model=byol, 
        ...
        save_loss(loss, LOSS_DIR, 'BYOL Pre-Training Loss')

    else:
        
        # define directories
        BASE_DIR = "byol_fine_tuned_models"
        SAVE_DIR = os.path.join(BASE_DIR, args.augmentation)
        if not os.path.exists(SAVE_DIR):
            os.makedirs(SAVE_DIR)
        ...
        if args.mode == "train":
            pretrained_weights = torch.load(f'byol_pretrained_models/{args.augmentation}/weights/byol_epoch_500.pth', map_location='cpu')
            ...
            linear_eval.backbone.load_state_dict({k.replace('online_encoder.0.', ''): v for k, v in pretrained_weights.items() if 'online_encoder.0' in k})
            full_eval.backbone.load_state_dict({k.replace('online_encoder.0.', ''): v for k, v in pretrained_weights.items() if 'online_encoder.0' in k})
            ...
            linear_weights = torch.load(f'byol_fine_tuned_models/{args.augmentation}/weights/byol_fine_tuned_linear.pth', map_location='cpu')
            full_weights = torch.load(f'byol_fine_tuned_models/{args.augmentation}/weights/byol_fine_tuned_full.pth', map_location='cpu')
...
def pretrain(model, augmentation, device, dataset, WEIGHTS_DIR):
    ...
    for epoch in range(EPOCHS_PRE):
        ...
        # train the model for 1 epoch and get the loss from BYOL
        epoch_loss = pretrain_epoch(model, optimizer, dataset, data1, data2, epoch, cosine_scheduler, device)
        loss_values.append(epoch_loss)
        
        if (epoch + 1) % 50 == 0: # only save the weights every 50 epochs
            logging.info(f'Epoch {epoch + 1}, Average Loss: {epoch_loss:.4f}')
            
            save_path = os.path.join(WEIGHTS_DIR, f'byol_epoch_{epoch + 1}.pth')
...
def pretrain_epoch(model, optimizer, dataset, data1, data2, epoch, cosine_scheduler, device):
    ...
    if data2:
        ...
        for batch_idx, (norm_batch, sup_batch) in enumerate(zip(dataloader1, dataloader2)):
            ...
            for name, param in model.online_encoder.named_parameters():
                model.accumulated_gradients[name] = model.accumulated_gradients[name].to(device)
                grad_device = param.device
                model.accumulated_gradients[name] += param.grad.to(grad_device)
            
            if (batch_idx + 1) % N == 0:  # average gradients every N steps and update target network once every N steps
                model.apply_accumulated_gradients(N)
                optimizer.step()
                optimizer.zero_grad()
...
def fine_tune(model, device, config, train_dataloader, val_dataloader, sweep, WEIGHTS_DIR):
    ...
    optimizer = torch.optim.SGD(model.parameters(), lr=config['lr'], momentum=0.9, weight_decay=0, nesterov=True)  # define Nesterov momentum optimizers
    ...
    save_path = os.path.join(WEIGHTS_DIR, f'byol_fine_tuned_{sweep}.pth')
    torch.save(model.state_dict(), save_path)
    logging.info(f'Model saved to {save_path}')
...
def save_fine_tune_loss(train_loss, val_loss, l_dir, sweep):
    ...
    save_loss_curve(train_loss, loss_curve_path, f"BYOL {sweep} sweep loss curves", val_loss)
...
def plot_confusion_matrix(true_labels, predicted_labels, classes, filename, augment, 
    ...
    ax.set_title(f'Confusion Matrix For BYOL {augment}', fontsize=fontsize+2)
...
def plot_auc_roc(true_labels, predicted_labels, augment, save_path):
    ...
    plt.title(f'Receiver Operating Characteristic (ROC) Curve for BYOL {augment}')
